...
def create_lrp_explanation(dataset,filename,class_id):
    ...
    #hot_one_label = sorted(dataset.top_predictions[file], key=itemgetter('score'))[-1]
    ...
    print(os.path.join(explanation_directory, 'elpr_' + str(class_id) + '_' + filename))
    print('waddup')
    plt.imsave(os.path.join(explanation_directory, 'elpr' + str(class_id) + '_' + filename),
               plot(attribution[0], xi=xi, dilation=.5, percentile=99, alpha=.2))
